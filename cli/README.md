...
- `npm run start -- psu record [--duration <sec>] [--output-csv <path>] [--output-perfetto <path>]` – record waveform data for the selected device context; CSV is written to stdout by default and non-data messages go to stderr. Use `--output-perfetto` to write a Perfetto trace with raw chunks + alert events (limits derive from machine type).
