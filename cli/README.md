...
- When multiple PSUs or loads are connected, use `psu1`, `psu2`, `load1`, etc. The unqualified `psu` name selects channel 0 only when all detected PSU channels share one controller; this keeps existing single-controller scripts working. With multiple controllers, or without PSU channel 0, `psu` remains ambiguous. The unqualified `load` name remains ambiguous with multiple loads.
